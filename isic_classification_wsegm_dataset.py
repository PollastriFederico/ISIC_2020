...
class ISIC(data.Dataset):
    ...
    data_root = '/nas/softechict-nas-1/fpollastri/data/ISIC_dataset/Task_3/'
    gtmasks = '/nas/softechict-nas-1/fpollastri/data/ISIC_dataset/Task_1/ground_truth/'
    mymasks = '/nas/softechict-nas-1/lcanalini/ISIC/2019/train_eval_test_2019/'
    ...
    def __init__(self, split_list=None, split_name='training_2019', classes=[[0, 1, 2, 3, 4, 5, 6, 7]], load=False,
                 size=(512, 512), segmentation_transform=None, transform=None, mask_transform=None):
        ...
        if load:
            ...
            self.imgs, self.masks = self.get_names(self.split_list)
    ...
    def __getitem__(self, index):
        ...
        if self.segmentation_transform is not None:
            image, mask = self.segmentation_transform(image, mask)
        if self.transform is not None:
            image = self.transform(image)
        if self.mask_transform is not None:
            mask = self.mask_transform(mask)

        return image, mask, self.lbls[index], self.imgs[index], self.masks[index]
    ...
    def get_names(cls, n_list):
        ...
        for n in n_list:
            ...
            if os.path.isfile(cls.gtmasks + n + '_segmentation.png'):
                masks.append(cls.gtmasks + n + '_segmentation.png')
            else:
                masks.append(cls.mymasks + n + '.png')
        return imgs, masks
