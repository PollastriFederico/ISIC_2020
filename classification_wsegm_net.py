...
import time
import numpy as np
from model import get_criterion, get_optimizer, get_scheduler, TemperatureScaling
from model import get_model_wsegm as get_model
from data import CutOut
from data import get_dataset_wsegm as get_dataset
...
class ClassifyNet:
    def __init__(self, net, dname, dropout, l_r, loss, optimizer, scheduler, size, batch_size, n_workers, augm_config,
        ...
        self.data_loader, self.test_data_loader, self.valid_data_loader = get_dataset(size=self.size,
...
def train(class_model, num_epochs, starting_e=0):
    for epoch in range(starting_e + 1, num_epochs):
        ...
        for idx, (x, y, target, _, _) in enumerate(class_model.data_loader):
            # measure data loading time
            # print("data time: " + str(time.time() - start_time))

            # compute output
            x = torch.cat((x, y), dim=1).to('cuda')
...
def eval(class_model, e_loader, predictions, labels, with_temp_scal=False, compute_separate_metrics_for_errors=False):
    with torch.no_grad():
        ...
        start_time = time.time()
        for idx, (x, y, target, img_name, _) in enumerate(e_loader):
            # measure data loading time
            # print("data time: " + str(time.time() - start_time))
            # compute output
            x = torch.cat((x, y), dim=1).to('cuda')
