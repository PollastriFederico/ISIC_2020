...
class birds_caltech_2011(data.Dataset):
    ...
    def __init__(self, directory, transform=None, target_transform=None, asnumpy=False, download=True, isTrain=True,
                 tam_image=300, interpolation='bilinear', padding='wrap', return_bounding_box=False):
        '''
        Arguments
    ...
    def __getitem__(self, index):
        ...
        if self.return_bounding_box:
            ...
            return sample, 0, torch.from_numpy(bbox)
    ...
    def _processing(self):
        ...
        for l1, l2, l3, l4 in zip(open(file_images), open(file_train_test), open(file_classes), open(file_bbox)):
            ...
            x, y, width, height = numpy.float32(x).astype('int32'), numpy.float32(y).astype('int32'), numpy.float32(
                width).astype('int32'), numpy.float32(height).astype('int32')
            ...
            if not self.return_bounding_box:
                ...
                image = numpy.pad(image, ((pad_size_row_l, pad_size_row_r), (pad_size_col_l, pad_size_col_r), (0, 0)),
                                  self.padding)
